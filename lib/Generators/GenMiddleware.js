...
\t}
    ...
    if (!options.api) {
        ...
        if (!fs.existsSync(`${appRoot}/Middleware/Controller/API/${name}Middleware.js`) || options.force) {
            fs.promises.writeFile(`${appRoot}/Middleware/Controller/API/${name}Middleware.js`, body)
                .catch(e => Logger.error(`API Middleware could not generate : ${e.message}`))
                .then(() => Logger.info(`${name}Middleware made`))
        } else Logger.info(`API Middleware ${name} already exists. Skipping stage`)
    }
